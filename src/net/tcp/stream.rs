...
mod tests {
    ...
    fn test_tcp_client() {
        ...
        let server_thread = thread::spawn(move || {
            ...
            println!("std server accept end");

            for _ in 0..TIMES {
                let mut buf = vec![0u8; REQUEST.len()];
                println!("std server read_exact start");
                stream.read_exact(&mut buf).expect("std read failed");
                println!("std server read_exact end");
                assert_eq!(REQUEST, buf);

                println!("std server write_all start");
                stream.write_all(RESPONSE).expect("std write failed");
                println!("std server write_all end");
            }
        });
        ...
        for _ in 0..TIMES {
            ...
            println!("orengine client recv end");
